...
import com.emrs.util.LoggerFactory;
import com.emrs.util.ProjectException;

/**
 * JPAOperator class
 * Used to excute sql clauses.
 * Provide divided page display
 * @author maybe
 *
 */
public class JPAOperator {
	...
	//Insert a record
	public static Object insertRecord(Object obj) {
		EntityManager em = null;
		try {
			em = JPAConnection.getEntityManager();
			em.getTransaction().begin();
			...
			try {
				Method method = obj.getClass().getMethod("getRid");
	...
	//Provide a common way to execute the sql for any type result
	...
	//Use map to pass the parameter of sql clauses
	public static List<?> noTypeQuery(String queryClause, Map<String, ?> parameters, 
			int pageNum, int pageSize) {
		return noTypeQuery(queryClause, parameters, 0, pageNum, pageSize);
	}
	
	//Provide divided page
	...
	//Query a single object through sql clause
	 ...
	 * @param pageSize
	 * The number of item each page display
	 * @return s
	 */
