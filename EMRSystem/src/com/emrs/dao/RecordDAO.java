...
public class RecordDAO {
	...
	public List<Record> getRecordByPatient(String pid) {
		...
		map.put("pid", pid);
		return JPAOperator.query(sql, map, Record.class, 1,
				Integer.MAX_VALUE / 2);
	}

	/*
	 * orderType: 0：自然排序 1: 按名字从a-z 2: 按名字从z-a 3: 按日期从近到远 4: 按日期从远到近 searchType:
	 * 0: 不搜索 1: 按病人名字搜索 2: 按病人id搜索
	 */
	public List<Record> searchRecord(int pageNum, int pageSize, int orderType,
			int searchType, String keyword, Date begin, Date end,
			String department, int gender) {
		...
		if (begin != null && !"".equals(begin) && end != null
			...
			map.put("begin", begin);
			map.put("end", end);
		} else if (begin != null && !"".equals(begin)) {
		...
		List<Record> lr = JPAOperator.query(sql, map, Record.class, pageNum,
				pageSize);
